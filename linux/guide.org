...
- [[#disk-and-partition-info][Disk and partition info]]
  ...
  - [[#check-file-sizes-in-a-directory][Check file sizes in a directory]]
- [[#rmlint][rmlint]]
...
#+BEGIN_SRC bash
df -Th
# For a certain dir. It will automatically find what is mounted given any path
df -hTP /var
# or
lsblk -f
#+END_SRC
...
# Get the sizes of all files and directories in the current
# directory sorted by size
du -hcs * | sort -hr

# Find all files in /var that ends with ".db", get the size of each
# file and then sort the result by size
find /var -type f -iname '*.db' -exec du -hcs {} + | sort -hr
#+END_SRC
