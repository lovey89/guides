...
- [[#clippy][Clippy]]
- [[#misc][Misc]]
  - [[#the-unimplemented-and-todo-macros][The ~unimplemented!()~ and ~todo!()~ macros]]
...
To define an enum representing the different standards for IP addresses you
...
fn print_number(x: u8) {
    match x {
        3 => println!("Three!"),
        7 => println!("Seven!"),
        22..=30 => println!("A value between 22 and 30 (inclusive)"),
...
If you remove ~pub~ from ~pub mod garden;~ the module will not be available to
any ancestors of ~main.rs~ (which there are none) but is still available to
~main.rs~ self (you can think of it as ~main.rs~ still knows it exists). But if
you remove ~pub~ from ~pub mod vegetables~ in ~garden.rs~, then it won't be
available to ~main.rs~ and the program wouldn't compile. Same for
~pub struct Asparagus {}~.
...
Instead of writing out the full paths to use items in other modules we can use
...
Const generics are generic arguments that range over constant values, rather
than types or lifetimes. This allows, for instance, types to be parameterized by
integers. In fact, there has been one example of const generic types since early
on in Rust's development: the array types [T; N], for some type T and N: usize.
However, there has previously been no way to abstract over arrays of an arbitrary
size: if you wanted to implement a trait for arrays of any size, you would have
to do so manually for each possible value.

Currently, const parameters may only be instantiated by const arguments of the
following forms:

- A standalone const parameter
- A literal (i.e. an integer, bool, or character)
- A concrete constant expression (enclosed by {}), involving no generic
  parameters
...
According to https://nindalf.com/posts/optimising-rust/#4-use-const-generics
...
when it was created (discovered it by myself).
...
When we enter ~*y~, behind the scenes Rust actually runs ~*(y.deref())~. The
...
fn main() {
    // Observation: The mutex is not mutable but can still be updated
    // interior mutability
    let m = Mutex::new(5);
...
safely~ and inform us that ~the trait `Send` is not implemented for
`Rc<Mutex<i32>>`~.
...
~Message::Hello~'s ~id~ field is within the range ~3..=7~. We also want to bind
...
#+BEGIN_SRC bash :noeval
cargo clippy -- -D warnings -W clippy::all -W clippy::correctness -W clippy::complexity -W clippy::style -W clippy::suspicious -W clippy::perf -W clippy::pedantic
#+END_SRC

* Misc

Can an immutable var become mutable? Write an example where a function returns
a ~String~ and assign it to both mutable and immutable variables.

** The ~unimplemented!()~ and ~todo!()~ macros

Both ~unimplemented!()~ and ~todo!()~ are shorthands for ~panic!()~ and can be
used for e.g. unreachable code to keep the type checker happy, or when you will
implement something later but just want the code to compile for now.
...
- ~scan_fmt!()~
- Benchmark with ~criterion~ or ~hyperfine~
