...
The last variable may be an empty list
...
If you try to unpack more variables than is possible you will get an error
...
Or if you want to add a collection of objects

#+BEGIN_SRC python :python python3 :results output
basket = {'apple', 'orange'}
print(basket)
basket.update(['banana', 'orange', 'pear'])
print(basket)
#+END_SRC
...
#+BEGIN_SRC python :python python3 :results output
s = "'{a_var}' and '{b_var}' and '{c_var}'"
input_vars = {'a_var': 'first var', 'b_var': 'second var', 'c_var': 'third_var', 'd_var': 'unused_var'}
print(s.format(**input_vars))
#+END_SRC
...
Old way that doesn't really do what we want
...
To make the data read-only you can use the ~frozen~ keyword:
...
class Person:
    ...
    def __post_init__(self):
        # To get around the read-only restriction
...
The ones named something like ~__name__~. The double underscore is sometime
