...
- [[#dockerfile][Dockerfile]]
  ...
  - [[#tags][Tags]]
- [[#stateful-containers][Stateful containers]]
  - [[#implicit-per-container-storage][Implicit per-container storage]]
  - [[#explicit-per-container-storage][Explicit per-container storage]]
  - [[#per-host-storage][Per host storage]]
...
- [[#other-examples][Other examples]]
  ...
  - [[#audio-in-docker][Audio in docker]]
  - [[#workaround-when-application-is-listening-to-localhost-instead-of-0000][Workaround when application is listening to ~localhost~ instead of ~0.0.0.0~]]
  - [[#install-custom-certs-and-use-proxies-in-java-applications-in-a-container][Install custom certs and use proxies in java applications in a container]]
...
- Directories are found under ~/var/lib/docker/volumes~ on the host
- Becomes unavailable if the container crashes or is deleted or host crashes
- Can't be shared with other containers

*** Implicit container storage example

Run

#+BEGIN_SRC bash
docker container run -it --name volumetest1 ubuntu bash
#+END_SRC

to create a container. The command

#+BEGIN_SRC bash
docker container inspect --format '{{json .Mounts}}' volumetest1
#+END_SRC

returns no volume information.

Navigate to ~08_implicit_volume~ where I have prepared a [[file:08_implicit_volume/Dockerfile][Dockerfile]] and run

#+BEGIN_SRC bash
docker image build -t implicit_volume_image .
#+END_SRC

Then launch it

#+BEGIN_SRC bash
docker container run -it --name volumetest2 implicit_volume_image bash
#+END_SRC

and run the command

#+BEGIN_SRC bash
docker container inspect --format '{{json .Mounts}}' volumetest2
#+END_SRC

Now we will see the place where the volume is mapped under
~/var/lib/docker/volumes~. In your container run:

#+BEGIN_SRC bash
echo "File content" > /my/test/dir/filename.txt
#+END_SRC

You will see that the file ~filename.txt~ was created under the directory given
by the ~inspect~ command.

Remove the containers

#+BEGIN_SRC bash
docker container rm -v volumetest1 volumetest2
#+END_SRC

The ~-v~ option also removes the volumes associated with the container.
Otherwise they will stay.

To see what volumes exists you can run

#+BEGIN_SRC bash
docker volume ls
#+END_SRC

TODO: What if I stop and start the container? Will the mapping stay?

** Explicit per-container storage

- Volumes are explicitly created (using the ~volume~ command)
- Named directory found under ~/var/lib/docker/volumes~ on the host
- Becomes unavailable if the container crashes or is deleted or host crashes
- Can't be shared with other containers

*** Explicit container storage example
...
You will see it show up both when you run
...
We reuse the container that we created [[*Implicit container storage example][previously]]. Mount the volume using the
~-v~ option

#+BEGIN_SRC bash
docker container run -it --name volumetest3 -v my_named_volume:/my/test/dir implicit_volume_image bash
#+END_SRC

In another terminal run:

#+BEGIN_SRC bash
docker container inspect --format '{{json .Mounts}}' volumetest3
#+END_SRC

and you will see that the directory is mounted correctly.

When we run

#+BEGIN_SRC bash
docker container rm -v volumetest3
#+END_SRC

the volume is for some reason not removed. We have to remove it explicitly
...
- Data is stored in a named directory on the host
- Available if the container crashes or is deleted
- Unavailable if the host crashes
- Multiple containers can read/write to the volume
- Any changes to the volume are not part of the container

*** Example

Navigate to any directory and create a directory called ~my_test_dir~ and
create a file ~my_file.txt~ inside it. Without navigating to that directory run

#+BEGIN_SRC bash
docker container run -it --name volumetest4 -v "$(pwd)/my_test_dir":/my/test/dir implicit_volume_image bash
#+END_SRC

In your container check the content of ~/my/test/dir/~:

#+BEGIN_SRC
root@bcdf351a0414:/# ll /my/test/dir/
total 12
drwxrwxr-x. 2 1000 1000 4096 Oct 18 21:23 ./
drwxr-xr-x. 3 root root 4096 Oct 18 21:21 ../
-rw-rw-r--. 1 1000 1000    9 Oct 18 21:23 my_file.txt
root@bcdf351a0414:/#
#+END_SRC

Try creating and removing files both from the container and host machine and see
how the content changes.

Also try running

#+BEGIN_SRC bash
docker container inspect --format '{{json .Mounts}}' volumetest4
#+END_SRC

and you will see the current mapping.

Detach from the container and remove it

#+BEGIN_SRC bash
docker container rm -v volumetest4
#+END_SRC
...
In [[file:firefox-gui-audio][firefox-gui-audio]] I have written a [[file:firefox-gui-audio/Dockerfile][Dockerfile]] which installs firefox. Build
the ~Dockerfile~:
...
Run the following command to allow ~docker~ to use the hosts ~X~ server:

#+BEGIN_SRC bash :noeval
xhost +local:
#+END_SRC

To start it with a gui, run:

#+BEGIN_SRC bash :noeval
docker run --rm -it \
  --env DISPLAY=$DISPLAY \
  -v /tmp/.X11-unix:/tmp/.X11-unix \
  firefox:latest firefox https://www.youtube.com/watch?v=OyWQro6q7qo
#+END_SRC

~--env DISPLAY=$DISPLAY~ and ~-v /tmp/.X11-unix:/tmp/.X11-unix~ are needed to
give ~docker~ access to the ~X~ server. A window is opened and you can start a
clip on youtube, but there is no sound. Read next section how to give ~docker~
access to ~pulseaudio~

** Audio in docker

The Dockerfile from the previous section also installs ~pulseaudio~ which is
needed to get sound from the container (at least if you follow these
instructions). First run the following commands to create a ~pulseaudio~ socket
and a ~pulseaudio.client.conf~ file.
...
The container needs to run as the same user as on the host system so we include
~-u~ flag to set user and group. We also mount ~/etc/passwd~ and ~/etc/group~ so
we get all information about the user. We also need a ~$HOME~ directory for this
user in the container, otherwise ~firefox~ will complain (an empty directory
seems to be fine, so we use the ~--tmpfs~ option).
...
A window will open and you can play a video with audio. Instead of mounting
~/etc/passwd~ and ~/etc/group~ you can also just set the ~$HOME~ environment
variable:
...
*** In fedora with ~pipewire~ and ~pipewire-pulse~

Got it working after reading:

- https://joonas.fi/2020/12/audio-in-docker-containers-linux-audio-subsystems-spotifyd/
- https://github.com/joonas-fi/spotifyd-docker

Maybe the following command also work in Ubuntu. You don't have to run prepare
the host system in any way for audio.
...
docker run --rm -it \
  ...
  --tmpfs "$HOME" \
  --env PULSE_SERVER=unix:/tmp/pulseaudio.socket \
  --volume /run/user/$UID/pulse/native:/tmp/pulseaudio.socket \
  --env DISPLAY=$DISPLAY \
  -v /tmp/.X11-unix:/tmp/.X11-unix \
  firefox:latest firefox https://www.youtube.com/watch?v=OyWQro6q7qo
#+END_SRC

** Workaround when application is listening to ~localhost~ instead of ~0.0.0.0~

In [[file:localhost-workaround][localhost-workaround]] I have written a [[file:localhost-workaround/Dockerfile][Dockerfile]] which installs ~python3~ and
~nftables~. Build the ~Dockerfile~:

#+BEGIN_SRC bash :noeval
docker build -t localhost-wa:latest .
#+END_SRC
...
The server is by default listening to ~0.0.0.0~. Navigate to
[[http://localhost:8000/]] and see that you are presented a directory listing.

All applications are not designed to listen to anything else but ~localhost~,
e.g the android emulator. Close the server and start it so it listens to
~127.0.0.1~ instead:
...
Again navigate to [[http://localhost:8000/]] and see that you can't reach it. You
...
In this case the container will share network layer with the host system. A
downside with this is that you can't run multiple identical containers
anylonger, since the ports can no longer be mapped (in fact ~-p 8000:8000~ have
no effect in the example above since all ports are exposed anyway).

Another way is to use ~nftables~ to redirect all incoming traffic on ~eth0~ to
~localhost~:

#+BEGIN_SRC bash :noeval
docker run --rm -it \
  -p 8000:8000 \
  -v "$PWD/entrypoint:/entrypoint:ro" \
...
In this example I have defined my own [[file:localhost-workaround/entrypoint][entrypoint script]] which makes all the
necessary ~nftables~ settings. This script is mounted and then referred to with
the ~--entrypoint~ option. To do the required settings, the container needs to
run with ~--privileged~.

** Install custom certs and use proxies in java applications in a container

This is good to know when running in a corporate network:

#+BEGIN_SRC
# CONFIGURE PROXY
http_proxy="http://proxy.com:8080"
https_proxy="http://proxy.com:8080"
# 169.254.169.254 is used by different cloud providers
no_proxy="localhost,127.0.0.1,example.com,169.254.169.254"

# Also check the "-Djava.net.useSystemProxies=true" setting to see if it respects http_proxy, https_proxy and no_proxy
# "-Dhttp.proxyPassword=" and "-Dhttps.proxyPassword=" may also be needed
JAVA_PROXY=${JAVA_PROXY:-"-Dhttp.proxyHost=proxy.com -Dhttp.proxyPort=8080 -Dhttps.proxyHost=proxy.com -Dhttps.proxyPort=8080 -Dhttp.nonProxyHosts='localhost|127.0.0.1|example.com|169.254.169.254'"}
JAVA_OPTS=${JAVA_OPTS:-"extra-options $JAVA_PROXY"}

# CONFIGURE CUSTOM CERTS
curl https://pki.example.com/pki/cert.pem.crt -o /usr/share/path/to/cert.crt
keytool -noprompt -storepass $JAVA_KEYSTORE_PASSWORD -import -alias an_alias.crt -cacerts -file /usr/share/path/to/cert.crt
#+END_SRC
