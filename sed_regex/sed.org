...
name of a label (which is defined as a string directly followed by a ~:~). If
you haven't provided a label to the ~b~ command it will branch to the end of the
script. This is nice to end work with the current row and start with a new from
top.
