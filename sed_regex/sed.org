...
row. To replace all occurences see the section about [[#global_replacement][global replacement]] below.
...
section about [[#print_flag][print flag]] above). So if you use both the ~-n~ option and the ~p~
