...
The file [[06_docker-compose-python-postgresql/docker-compose.yml]] will start up
two containers that will be able to communicate with each other. See the
comments inside that file for details.

It depends on [[06_docker-compose-python-postgresql/Dockerfile]] which in turn
depends on [[06_docker-compose-python-postgresql/postgresprog.py]].
